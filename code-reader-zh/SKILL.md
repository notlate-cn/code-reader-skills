...
- 代码 > 10000 行 / 文件数 > 20：分层并行（先模块级扫描，再章节级并行）
...
| **2. 框架准备** | 主 Agent | 基于项目地图，生成大纲和核心概念列表 | `框架.json` |
| **3. 任务分发** | 主 Agent | 为每个章节/模块创建独立任务，**附上具体文件路径列表** | 任务列表 |
| **4. 并行处理** | 子 Agents | 每个子 Agent 专注一个章节，读取指定文件深度生成 | `章节-N.md` |
| **5. 覆盖率校验** | 主 Agent | **对比项目地图，检查哪些文件/模块未被覆盖** | 覆盖率报告 |
| **6. 结果汇总** | 主 Agent | 合并所有章节，统一格式 | `完整分析.md` |
...
- **你负责的文件：** [明确列出具体文件路径，例如：src/auth.py, src/utils/crypto.py]
- **其他模块摘要：** [简要说明其他模块的职责，避免重复分析]

## 你的任务
你是专门负责"**[章节名称]**"章节的分析专家。**必须先用 Read 工具读取上方指定的文件**，基于实际代码生成内容，不能凭记忆或假设。

## 强制读取步骤
1. 用 Read 工具读取"你负责的文件"中列出的每一个文件
2. 确认文件内容后再开始分析
3. 如果文件不存在，说明原因并分析最近的替代文件

## 输出要求
1. **内容深度：** 本章节至少 [X] 字
2. **WHY 分析：** 每个关键点必须回答 3 个 WHY
3. **代码注释：** 使用场景/步骤 + WHY 风格
4. **引用来源：** 提供权威参考链接
5. **独立性：** 生成完整独立的章节内容，不需要引用其他章节
6. **覆盖完整性：** 你负责的文件中，每个公共函数/类都必须提及

## 输出格式
直接输出 Markdown 格式的章节内容，以 `## [章节名称]` 开头。
...
2. **覆盖率校验（关键步骤，防止信息丢失）**
   ...
   | 文件路径 | 是否被分析 | 分析章节 | 备注 |
   |---------|-----------|---------|------|
   | src/auth.py | ✅ | 章节_5 | 核心认证逻辑 |
   | src/utils/crypto.py | ✅ | 章节_5 | 加密工具 |
   | src/models/user.py | ❌ | - | 未覆盖，需补充 |
   | tests/test_auth.py | ✅ | 章节_6 | 测试分析 |
   ...
   ### 未覆盖内容处理
   [列出所有未覆盖的文件，并补充简要分析或说明跳过原因]
   ```

   **处理未覆盖内容：**
   - 重要文件（核心业务逻辑）：立即补充分析
   - 次要文件（配置、工具）：在依赖关系章节中简要提及
   - 测试文件：确认已在测试用例分析章节中覆盖
...
5. **深度验证**
   ...
   - WHY 分析充分
   - 执行示例完整
   - 覆盖率达到预期（核心模块 100%，整体 ≥ 80%）
```

#### 实现伪代码

```
函数：ParallelDeepMode(项目路径, 工作目录):

  // ========== 阶段 0: 建立完整项目地图（防止信息丢失的关键步骤）==========
  项目地图 = {
    "所有文件": 递归枚举所有源码文件(项目路径),  // 绝对路径列表
    "目录结构": 生成目录树(项目路径),
    "文件统计": {
      "总文件数": len(所有文件),
      "总行数": 统计所有文件行数(所有文件),
    ...
    "入口文件": 识别入口文件(所有文件),  // main, index, __init__ 等
    "核心模块": 识别核心模块(所有文件),
    "测试文件": 过滤测试文件(所有文件),
    "配置文件": 过滤配置文件(所有文件)
  }

  写入文件(f"{工作目录}/项目地图.md", 格式化项目地图(项目地图))

  // ========== 阶段 1: 框架准备 ==========
  // 注意：基于完整项目地图生成框架，不能只看部分文件
  框架 = {
    "代码名称": 提取名称(项目路径),
    "编程语言": 识别语言(项目地图),
    "代码规模": 项目地图["文件统计"]["总行数"],
    "核心概念": 提取核心概念(项目地图["核心模块"]),
    "模块职责表": 为每个核心模块生成一句话职责描述,  // 共享给子 Agent
    "章节到文件映射": {  // 明确指定每个章节负责哪些文件
      "背景与动机": [入口文件, README, 主配置文件],
      "核心概念": [核心模块列表],
      "算法与理论": [包含算法的文件],
      "设计模式": [核心模块列表],
      "关键代码解析": [最重要的5-10个文件],
  ...
  对于每个 章节 in 框架["章节到文件映射"]:
    负责文件 = 框架["章节到文件映射"][章节]
    任务描述 = 生成任务模板(
      章节, 框架,
      负责文件=负责文件,           // 明确告知要读取哪些文件
      模块职责表=框架["模块职责表"]  // 避免子 Agent 重复分析已有模块
    )
  ...
  // ========== 阶段 4: 覆盖率校验 ==========
  已分析文件 = 从所有章节文件中提取已引用的文件路径()
  未覆盖文件 = 项目地图["核心模块"] - 已分析文件

  if len(未覆盖文件) > 0:
    // 补充分析未覆盖的重要文件
    对于每个 文件 in 未覆盖文件:
      if 文件.是核心模块():
        补充分析 = 创建Agent(任务=f"分析 {文件}，写入覆盖补充章节")
        章节文件列表.append(补充分析.输出文件)

  // ========== 阶段 5: 结果汇总 ==========
  完整文档 = "# {框架['代码名称']} 完全掌握分析\n\n"
  完整文档 += "## 覆盖率摘要\n"
  完整文档 += f"- 总文件数：{项目地图['文件统计']['总文件数']}\n"
  完整文档 += f"- 已覆盖核心模块：{len(已分析文件)}/{len(项目地图['核心模块'])}\n"
  完整文档 += "## 理解验证状态\n\n"
  完整文档 += 生成验证表格(框架) + "\n\n"
...
| 类别 | 文件路径 | 行数 | 职责摘要 |
|------|---------|------|---------|
| 核心逻辑 | src/auth.py | 350 | 用户认证与授权 |
| 核心逻辑 | src/db.py | 180 | 数据库操作封装 |
| 工具模块 | src/utils/crypto.py | 90 | 加密/哈希工具 |
| 测试 | tests/test_auth.py | 210 | 认证功能测试 |
| 配置 | config/settings.py | 60 | 应用配置 |
...
**注意：此步骤必须在分析任何具体代码之前完成，是防止遗漏信息的基础。**
...
**分析文档必须读起来像一位有经验的工程师在给你讲解，而不是照本宣科地堆砌术语。**

### 核心要求

**1. 用口语化的引导句开头，而非直接甩定义**

❌ 生硬：
> `authenticate_user` 函数负责执行用户身份验证流程，通过接收 username 和 password 参数...

✅ 自然：
> 这个函数是整个认证系统的"门卫"——用户每次登录都要经过它。它做的事情很简单：先查用户存不存在，存在的话再核对密码，都对了才放行。

**2. 解释"为什么"时，先说结论，再展开原因**

❌ 生硬：
> 由于 bcrypt 算法具有自适应性哈希函数特性，其计算成本可通过 cost factor 参数进行动态调整，因此被选用。

✅ 自然：
> 用 bcrypt 而不用 MD5，根本原因是：MD5 太快了。一台普通电脑每秒能算几十亿个 MD5，暴力破解轻而易举。bcrypt 故意设计得很慢，还能随硬件升级调整"慢的程度"。

**3. 类比和比喻是你的朋友**

- 讲锁机制 → 比作"读写锁就像图书馆的阅览规则：多人可以同时看书，但有人要修改时所有人都得等"
- 讲缓存 → "缓存就是把常用的东西放桌上，而不是每次都去仓库取"
- 讲递归 → "就像查字典，遇到不认识的词，再去查那个词的解释"
...
✅ 自然注释：
```python
# 核对密码：把用户输入的密码重新哈希一遍，看结果和数据库里存的是否一致
# （不能直接比明文，因为数据库里根本没存明文）
if verify_password(password, user.password_hash):
```
...
先用一句话给出直觉理解，再补充技术细节：
> JWT 本质上是一张"盖了章的通行证"。你登录成功后，服务器给你一张证，以后你每次请求带着它，服务器验一下章是不是自己盖的，是的话就放行。技术上它是三段 Base64 编码拼起来的字符串，分别是 header、payload 和 signature。

**6. 标注"这里容易踩坑"**

不要只说正确做法，顺手点出新手常犯的错误：
> ⚠️ **常见误区：** 很多人以为 JWT 是加密的，其实不是——它只是签名，payload 内容 Base64 解码后任何人都能看。所以千万不要把密码、银行卡号之类的敏感信息放进去。

### 禁止的写法

| 禁止 | 原因 |
|------|------|
| 大段罗列术语不加解释 | 读者不知道这些词之间有什么关系 |
| "该函数实现了 X 功能" | 等于没说，读者已经知道它实现了 X |
| 每段都用相同句式开头 | 读起来像机器生成的，容易疲劳 |
| WHY 解释只有一句话 | 解释不充分，没有真正回答为什么 |
| 照抄官方文档的措辞 | 官方文档已经有了，复读没有价值 |

### 自检问题

写完后问自己：
- 这段话，一个刚入行的工程师能看懂吗？
- 我有没有解释"为什么"，还是只说了"是什么"？
- 有没有让读者有"原来如此！"的感觉？
...
| 项目规模 | 推荐模式 | 生成策略 | 文件结构 | 防丢失措施 |
|---------|---------|---------|---------|----------|
| < 500 行 | Quick/Standard | 单文档 | `[名称]-分析.md` | 无需额外措施 |
| 500-2000 行 | Standard | 单文档（可能较长） | `[名称]-分析.md` | 建立文件清单 |
| 2000-10000 行 / 文件数 ≤ 20 | Deep（策略 B） | 项目地图 + 并行章节 | 项目地图.md + 多章节 → 1个最终文档 | 项目地图 + 覆盖率校验 |
| > 10000 行 / 文件数 > 20 | Deep（策略 C） | 分层并行 | 模块摘要 + 章节 → 1个最终文档 | 模块级扫描 + 章节级校验 |
...
cat > code-analysis/00-framework.json << 'EOF'
{
  ...
  "chapter_file_mapping": {
    "背景与动机": ["path/to/main.py", "README.md"],
    "核心概念": ["path/to/core1.py", "path/to/core2.py"],
    "算法理论": ["path/to/algo.py"],
    "设计模式": ["path/to/core1.py", "path/to/core2.py"],
    "代码解析": ["path/to/most_important.py", "..."],
    "测试用例分析": ["tests/test_a.py", "tests/test_b.py"],
    "应用迁移": ["path/to/core1.py"],
    "依赖关系": ["requirements.txt", "package.json", "...全部文件列表"]
  }
}
EOF
```

#### 步骤 4: 创建并行子 Agent（附带明确文件路径）
```
...
Task(
  ...
  - 语言：{language}
  - 核心概念：{core_concepts}
  - 模块职责摘要：{module_responsibilities}  # 避免重复分析

  ## 你必须读取的文件（按优先级）
  {chapter_file_list}  # 从 chapter_file_mapping 中取对应章节的文件列表

  ## 强制步骤
  1. 用 Read 工具依次读取上方每个文件
  2. 读取完所有文件后再开始分析
  3. 分析内容必须引用实际代码行，不能凭记忆
...
1. 读取 code-analysis/项目地图.md 中的完整文件列表
2. 扫描 code-analysis/chapters/*.md，提取所有引用的文件路径
3. 生成覆盖率报告：
   - 核心模块覆盖率目标：100%
   - 整体覆盖率目标：≥ 80%
4. 对未覆盖的核心模块，创建补充分析任务
5. 读取 code-analysis/00-framework.json
6. 按顺序合并所有章节文件
7. 写入 {项目名}-完全掌握分析.md
```
...
阶段 1：模块级扫描（串行）
- 将项目按目录/功能分为 3-8 个模块
- 为每个模块生成独立的"模块摘要"（子 Agent 并行）

阶段 2：章节级分析（并行）
- 每个章节的子 Agent 获取：
  a. 完整项目地图
  b. 自己负责的模块摘要
  c. 需要深入阅读的关键文件列表（从模块摘要中筛选）
