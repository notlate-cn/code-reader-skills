...
description: 基于认知科学的源代码深度理解助手（中文改进版）。支持四种分析模式：Quick（快速概览，5-10分钟）、Standard（标准理解，15-20分钟）、Deep（深度掌握，30分钟以上）、Parallel Deep（并行深度，大型项目，子Agent并行处理）。结合精细询问法、自我解释测试和检索练习，帮助真正理解和掌握代码。触发词："深入分析这段代码"、"帮我理解这个算法"、"解释这个实现原理"、"为什么这样写代码"、"快速分析这段代码"、"并行分析这个项目"。
---

# 源代码深度理解分析器 v2.3 (Code Deep Understanding Analyzer - 中文版)

基于认知科学研究的专业代码分析工具，支持四种分析深度，确保真正理解代码，而非产生流畅幻觉。

## 四种分析模式

| 用户意图 | 推荐模式 | 触发词示例 | 分析时长 |
|---------|---------|-----------|---------|
| 快速浏览/代码审查 | Quick Mode | "快速分析"、"简单看看"、"这段代码是干嘛的" | 5-10 分钟 |
| 学习理解/技术调研 | Standard Mode ⭐ | "深入分析"、"帮我理解"、"解释原理" | 15-20 分钟 |
| 面试准备/完全掌握 | Deep Mode | "彻底分析"、"我要掌握这个"、"面试相关" | 30+ 分钟 |
| 大型项目/复杂代码库 | **Parallel Deep Mode** 🚀 | "并行分析"、"大型项目分析"、"完整项目理解" | 按项目规模 |

**默认使用 Standard Mode，除非用户明确指定其他模式。**

**🚀 Parallel Deep Mode 专为大型项目设计，使用子 Agent 并行处理各章节，确保每个章节都有足够深度。**
...
### Deep Mode 输出结构（30+ 分钟）- 渐进式生成版

**采用渐进式生成策略，确保每个章节都有足够深度：**
...
### 🚀 Parallel Deep Mode 输出结构（大型项目专用）

**适用于代码规模大、复杂度高的项目。采用并行 Agent 架构，确保每个章节都有足够深度。**
...
当用户使用以下表达时，自动选择 Parallel Deep Mode：
...
- "深度分析整个项目"
- "项目规模超过 5000 行"
- 或者检测到代码规模 > 5000 行时，建议使用 Parallel Mode

#### 并行策略的优势

| 对比项 | 传统 Deep Mode | Parallel Deep Mode |
|-------|----------------|-------------------|
| 执行方式 | 顺序生成各章节 | 并行生成各章节 |
| 每章深度 | 受上下文限制，可能简略 | 独立上下文，充分展开 |
| 总耗时 | 较长 | 较短（并行加速） |
| 质量保证 | 依赖单次输出深度 | 每章独立质量检查 |
| 适用场景 | 中小型代码 | 大型项目、复杂代码库 |

**渐进式生成流程（Deep Mode 专用）：**

| 阶段 | 操作 | 确保深度的机制 |
|-----|------|--------------|
| **1. 框架生成** | 生成完整目录 + 每章占位符 | 提前规划结构，避免遗漏 |
| **2. 逐章填充** | 按顺序处理每个章节 | 单次专注一个章节，充分展开 |
| **3. 深度检查** | 每章完成后自检 | 验证是否满足 Deep Mode 标准 |
| **4. 补充强化** | 薄弱章节追加内容 | 针对性补充不足部分 |
...
| **Deep** | 渐进式 Write | 1-2 | 面试准备、完全掌握 |
| **Parallel Deep** | 多 Agent 并行 + 汇总 | 多个章节 → 1 个最终文档 | 大型项目、复杂代码库 |
...
2. **生成方式（Token 优化流程）**
   ...
   **方式三：渐进式生成（Deep Mode 专用）**
   ```
   适用于：代码复杂、分析篇幅预计超过 5000 字的场景

   具体流程参见前文 "Deep Mode 输出结构 - 渐进式生成流程" 章节
   ```

   **方式四：并行生成（Parallel Deep Mode 专用）🚀**
   ```
   适用于：大型项目（代码量 > 5000 行）、复杂代码库
   ...
   示例 Task 调用：
   Task(
     description: "分析背景与动机章节",
     prompt: "你是背景与动机分析专家。请深度分析以下代码的背景与动机...[具体指令]",
...
| 2000-10000 行 | Deep | 分模块 | `[项目]-概述.md` + `[模块]-分析.md` (3-5个) |
| > 10000 行 | **Parallel Deep** 🚀 | 并行章节 | 多个临时章节 → 1个最终文档 |
| > 50000 行 | **Parallel Deep** 🚀 | 分层并行 | 模块级并行 + 章节级并行 |

**重要：不要在对话中输出完整分析结果，直接写入文件，仅输出摘要！**

---

### 🚀 Parallel Deep Mode 实现指南（给 Claude 的具体指令）

当检测到用户需要 Parallel Deep Mode 时，按以下步骤执行：
...
**Deep Mode 渐进式生成时，每章完成后必须通过以下检查：**
