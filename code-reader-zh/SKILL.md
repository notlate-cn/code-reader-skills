...
| Agent 类型 | 适用场景 | 示例 |
|-----------|---------|------|
| `Explore` | 需要多轮探索、发现隐藏关系 | 快速概览、概念网络、依赖关系探索 |
| `general-purpose` | 单次分析、需要精确输出 | 算法分析、设计模式、代码解析、质量验证 |
...
```markdown
# Standard/Deep Mode 并行优化

1. 串行：快速概览 → 背景动机 → 概念网络
2. 并行：[算法分析, 设计模式分析]
3. 串行：关键代码解析（依赖前面结果）
4. 并行：[应用迁移场景1, 应用迁移场景2]
5. 串行：质量验证

# 伪代码示例
# 步骤 4-5 并行
[算法结果, 模式结果] = ParallelRun(
  Task("算法分析", {概念}),
  Task("设计模式", {概念})
)
...
|------|------|
| Quick Mode | ❌ 串行即可，内容少 |
| Standard Mode + 简单代码 | ❌ 串行即可 |
| Standard Mode + 复杂代码 | ✅ 适度并行 |
| Deep Mode | ✅ 大量并行 + 渐进生成 |

**Token 注意：**
- 子 Agent 会增加总 token 消耗（上下文重复）
- 仅在并行收益 > 开销时使用
- Deep Mode 渐进式生成时，可考虑每章节用独立子 Agent
...
2. **生成方式（Token 优化流程）**
   ...
   **方式三：超长分析渐进式生成（Deep Mode 专用）**
   ```
   适用于：代码复杂、分析篇幅超过 5000 字的场景
   ...
   步骤 2: 逐章填充（每章独立生成，确保深度）
   ```
   # 读取当前文件
   当前内容 = Read([名称]-分析.md)

   # 生成单个章节（专注深度）
   章节1_内容 = 深度生成("1. 快速概览", 代码)
   # 要求：至少 300 字，包含语言/规模/依赖/类型

   # 替换占位符
   新内容 = 当前内容.replace("[待填充...]", 章节1_内容)
   Write([名称]-分析.md, 新内容)
